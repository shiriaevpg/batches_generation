// modification of asd_mutator with metric / len_batch and center of mass
...
class MutatorCMUnit {
 public:
  ...
  MutatorCMUnit(const std::vector<Order>& orders, const std::vector<size_t>& trace,
  ...
  MutatorCMUnit(const MutatorCMUnit&) = default;
...
//  Gainer gainer_;  // для стресс тестов
  ...
  long double GetAbsDiff(const Mutation& mutation);
...
MutatorCMUnit::Mutation::Mutation(MutatorCMUnit::MutationType mutation_type, size_t pos)
    : mutation_type(mutation_type), number1(pos) {}

MutatorCMUnit::Mutation::Mutation(MutatorCMUnit::MutationType mutation_type,
                              const std::pair<size_t, size_t>& swap_poses)
    : mutation_type(mutation_type), number1(swap_poses.first), number2(swap_poses.second) {}

BatchT MutatorCMUnit::GetBatch() {
...
MutatorCMUnit::MutatorCMUnit(const std::vector<Order>& orders, const std::vector<size_t>& trace,
...
void MutatorCMUnit::Remove(size_t id) {
  used_.erase(id);
  not_used_.insert(id);
...
void MutatorCMUnit::Add(size_t id) {
...
void MutatorCMUnit::Change(const MutatorCMUnit::Mutation& mutation) {
  ...
  curr_gain_ += GetAbsDiff(mutation);
}

MutatorCMUnit::Mutation MutatorCMUnit::operator()(long double temp) const {
...
const Point& MutatorCMUnit::GetNextSrc(std::map<size_t, size_t>::iterator it) const {
  if ((++it) == src_id_.end()) {
...
const Point& MutatorCMUnit::GetNextDest(std::map<size_t, size_t>::iterator it) const {
  if ((++it) == dest_id_.end()) {
...
const Point& MutatorCMUnit::GetPrevSrc(std::map<size_t, size_t>::iterator it) const {
  if (it == src_id_.begin()) {
    return kDefaultPoint;
  }
  return orders_[(--it)->second].source;
...
const Point& MutatorCMUnit::GetPrevDest(std::map<size_t, size_t>::iterator it) const {
...
long double MutatorCMUnit::GetAbsDiff(const MutatorCMUnit::Mutation& mutation) {
...
long double MutatorCMUnit::GetDiff(const MutatorCMUnit::Mutation& mutation) {
  if (mutation.mutation_type == MutationType::kRemove) {
    return (GetAbsDiff(mutation) + curr_gain_ / used_.size()) / (used_.size() - 1);
  }
  if (mutation.mutation_type == MutationType::kAdd) {
    return (GetAbsDiff(mutation) - curr_gain_ / used_.size()) / (used_.size() + 1);
  }
  return GetAbsDiff(mutation);
}

long double MutatorCMUnit::GetRemoveDiff(size_t id) {
...
long double MutatorCMUnit::GetAddDiff(size_t id) {
...
long double MutatorCMUnit::GetSwapSrcDiff(size_t pos1, size_t pos2) {
...
long double MutatorCMUnit::GetSwapDestDiff(size_t pos1, size_t pos2) {
...
long double MutatorCMUnit::GetSwapDiff(
...
long double MutatorCMUnit::GetSwapNearDiff(const Point& prev, const Point& point1, const Point& point2, const Point& next) {
  return GetLength(prev, point1) + GetLength(point2, next) -
      (GetLength(prev, point2) + GetLength(point1, next));
}

long double MutatorCMUnit::GetProb(const MutatorCMUnit::Mutation& mutation, long double temp) {
  return std::min(std::exp(GetAbsDiff(mutation) / temp), static_cast<long double>(1));
}